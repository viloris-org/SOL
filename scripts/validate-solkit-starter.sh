#!/usr/bin/env sh
# Validate the template exactly as an external checkout would consume it.
set -eu

script_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
repo_root=$(CDPATH= cd -- "$script_dir/.." && pwd)
scratch_dir=$(mktemp -d "${TMPDIR:-/tmp}/solkit-starter.XXXXXX")
project_dir="$scratch_dir/solkit-starter"
