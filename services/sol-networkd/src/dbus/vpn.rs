...
impl VpnInterface {
    ...
    async fn connect(&self, profile_id: String) -> fdo::Result<()> {
        self.manager.connect_to_profile(&crate::profile::ProfileId(profile_id))
            .await
    ...
    async fn disconnect(&self, profile_id: String) -> fdo::Result<()> {
        self.manager.disconnect_profile(&crate::profile::ProfileId(profile_id))
            .await
            .map_err(|e| fdo::Error::Failed(e.to_string()))?;
        Ok(())
    }
    ...
    async fn create_wireguard(
        ...
        // Parse peers
        let peers_value = config.get("peers")
        ...
        let profile_id = self.manager.create_profile(profile).await
            .map_err(|e| fdo::Error::Failed(e.to_string()))?;
    ...
    async fn get_status(&self, _profile_id: String) -> fdo::Result<HashMap<String, zbus::zvariant::Value<'static>>> {
...
fn parse_peers(value: &zbus::zvariant::Value) -> Result<Vec<WireGuardPeer>, String> {
    let array = value.downcast_ref::<zbus::zvariant::Array>()
        .map_err(|_| "peers must be an array".to_string())?;

    let mut peers = Vec::new();
    for item in array.iter() {
        let dict = item.downcast_ref::<zbus::zvariant::Dict>()
        ...
        let public_key = peer_map.get("public_key")
            .and_then(|v| {
        ...
        let endpoint = peer_map.get("endpoint")
            .and_then(|v| {
                if let zbus::zvariant::Value::Str(s) = v {
        ...
        let allowed_ips = peer_map.get("allowed_ips")
        ...
        let persistent_keepalive = peer_map.get("persistent_keepalive")
...
fn wireguard_config_to_profile(config: &WireGuardConfig) -> crate::profile::vpn_profile::WireGuardProfile {
    use crate::profile::vpn_profile::{WireGuardProfile, WireGuardPeerProfile};

    WireGuardProfile {
        private_key_encrypted: config.private_key.as_bytes().to_vec(), // TODO: Actually encrypt
        address: config.address.clone(),
        listen_port: config.listen_port,
        peers: config.peers.iter().map(|p| WireGuardPeerProfile {
            name: None,
