//! A-1: Multiple concurrent toplevels round-trip.
//!
//! Spawns the compositor, creates 3 xdg_toplevel surfaces in sequence, acks
//! each configure, and exits 0 once all 3 are acked.
//!
//! Verifies: the compositor tracks multiple toplevels simultaneously and
//! delivers independent configure/ack round-trips for each (A-1).
...
#[test]
#[serial]
fn multiple_toplevels_round_trip() {
    ...
    let client_bin = build_bin("sol-compositor --example test-client-multi");
    let output = Command::new(client_bin)
        .env("WAYLAND_DISPLAY", &_session.socket)
        .env("XDG_RUNTIME_DIR", &_session.runtime_dir)
        .output()
        .expect("run test-client-multi");

    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    eprintln!("multi stdout: {stdout}");
    eprintln!("multi stderr: {stderr}");
    assert!(
        output.status.success(),
        "test-client-multi failed with {:?}",
        output.status
    );
    assert!(
        stderr.contains("3 toplevels acked") || stdout.contains("3 toplevels acked"),
        "test-client-multi did not report 3 toplevel round-trips"
    );
}
